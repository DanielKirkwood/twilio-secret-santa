...
async def solve_secret_santa_csp_for_group(group_id: int) -> dict[Person, Person] | None:
    problem = Problem()
    ...
    for person in members:
        all_except_me = [
            p for p in members if p.username != person.username]
        problem.addVariable(
            person, all_except_me)
    ...
    for person in members:
        ...
        if len(previous_receivers) > 0:
            problem.addConstraint(NotInSetConstraint(
                previous_receivers), [person])

        partner = person.partner
        if partner is not None:
            problem.addConstraint(
                lambda p: p != partner, [person])
